...
<div class="podcart">	
	...
	<div class="cartable" id="item_cart_{$purchase->new_variant_id}">
		...
		<div class="cartd carprice">
			<div class="varcen" style="display: none;">{$purchase->variant->price}</div>
			{if $purchase->variant->compare_price}
		...
		<div class="cartd carkolvo">
			<div class="amountposit">
				<div class="amount">
					<input class="add_input" name="amounts[{$purchase->new_variant_id}]" onchange="update_cart('{$purchase->new_variant_id}',$(this).val(),{$purchase->variant->price}{if $purchase->variant->compare_price},{$purchase->variant->compare_price}{/if});" type="text" value="{$purchase->amount}" min="1" max="10" data-variant="{$purchase->new_variant_id}" autocomplete="off" role="spinbutton">
	...
	<div class="carforms2">
		<div class="pad-b-30 pad-t-30">
			<div class="kupname">Выберите способ доставки:</div>
								...
								<li>
									<label class="andru and2" for=payment_{$delivery->id}{$payment_method->id}>
										...
										<input class="{if $payment_method@first}payment_first{/if}" type=radio name=payment_method_id value='{$payment_method->id}' {if $payment_method@first}checked{/if} id=payment_{$delivery->id}{$payment_method->id}>
												...
												{/if}
												<div class="pay">к оплате: {$total_price_with_delivery|convert:$payment_method->currency_id}&nbsp;{$all_currencies[$payment_method->currency_id]->sign}</div>
	...
	<div class="podsendcart mar-t-30 mar-b-50">
		<div class="sendcart">
			<div class="scitem">
				<ul class="export">
					<li>
						<input name="name" type="text" value="{$name|escape}" placeholder="Ваше ФИО *" required />
					</li>
					<li>
						<input name="email" type="email" value="{$email|escape}" placeholder="Ваш e-mail *" required />
					</li>
				</ul>
			</div>
			<div class="scitem">
				<ul class="export">
					<li>
						<input id="phonecart" name="phone" type="text" value="{$phone|escape}" placeholder="Номер телефона *" required />
					</li>
					<li>
    ...
    <center><div class="g-recaptcha" data-sitekey="{$settings->site_code|escape}" style="margin-bottom:20px;"></div></center>
...
{literal}
<script type="text/javascript">
function update_cart(new_variant_id,amount,price,old_price)
{
	$.ajax({
